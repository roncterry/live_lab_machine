...
get_build_type() {
  ...
  then
    ...
    case ${1} in
      -h|--help)
        echo
	echo "DESCRIPTION:  This script only initializes the cache"
	echo "              for a Kiwi image."
	echo
...
initialize_cache_only() {
  ...
  case ${KIWI_NEXT_GEN} in
    y|Y|yes|Yes|YES)
      echo "COMMAND: ${KIWI_CMD} compat --init-cache ${KIWI_EXPORT_DIR}/${KIWI_CONFIG_DIR}"
      echo
      ${KIWI_CMD} compat --init-cache ${KIWI_EXPORT_DIR}/${KIWI_CONFIG_DIR}
    ;;
    *)
      echo "COMMAND: ${KIWI_CMD} --init-cache ${KIWI_EXPORT_DIR}/${KIWI_CONFIG_DIR}"
      echo
      ${KIWI_CMD} --init-cache ${KIWI_EXPORT_DIR}/${KIWI_CONFIG_DIR}
    ;;
  esac
  echo
  echo "*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*"
  echo "*         Finished initializing cache for: ${BUILD_TYPE}"
  echo "*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*"
  echo
  echo
}
